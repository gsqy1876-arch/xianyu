        ...
        try:
            ...
            try:
                ...
                if message['3']['redReminder'] == '等待买家付款':
                    ...
                    # 自动发送网盘链接回复
                    paid_msg = AUTO_REPLY.get('paid_message')
                    if paid_msg:
                        await self.send_msg(websocket, user_id, user_id, paid_msg.strip())
                        logger.info(f'[{msg_time}] 【系统自动发出】已发送网盘链接给用户: {user_id}')
                    return
            except:
                pass
            ...
            # 检查是否启用普通聊天自动回复
            if not AUTO_REPLY.get('enable_chat_reply', False):
                logger.debug(f"[{msg_time}] 【系统】普通聊天自动回复已禁用，不回复")
                return
